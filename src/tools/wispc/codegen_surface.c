...
int emit_surfaces(FILE *o, Unit *u, CGCtx *ctx) {
    ...
    /* dwl_tags(labels=, pinned=) override the config.h defaults per-preset. */
    {
        const char *labels = NULL; size_t llen = 0;
        const char *pinned = NULL; size_t plen = 0;
        for (int i = 0; i < ctx->nsrc; i++) {
            if (ctx->srcs[i].drv->drv != DRV_DWL_TAGS) continue;
    ...
    int has_dwl_tags_src = 0;
    for (int i = 0; i < ctx->nsrc; i++)
        if (ctx->srcs[i].drv->drv == DRV_DWL_TAGS) has_dwl_tags_src = 1;
    ...
    for (int i = 0; i < nsurf; i++) {
        fprintf(o, "    for (int i = 0; i < __%s_nw; i++) if (__%s_widgets[i] == w) { render_%s(w); return; }\n",
                surf_names[i], surf_names[i], surf_names[i]);
    }
    fputs("    (void)w;\n}\n\n", o);
    ...
    /* dwl_tags setters: always emit so wl.c/ctl.c link cleanly under
     * WISP_HAS_BAR. When no dwl_tags source exists, the fanout still writes
     * the per-widget state but wispgen_dwl_tags_changed() is a no-op stub. */
    (void)has_dwl_tags_src;
    fputs("extern void wispgen_dwl_tags_changed(void);\n", o);
