...
A `source` is the only thing in wisp that changes on its own. There are 22 of
...
| `mpris()` | `title` | `artist` `status` `player` | session bus |
...
**`mpris()`** gives `title` `artist` `status` `player` off the session bus.
